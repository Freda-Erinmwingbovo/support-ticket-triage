# app.py ← YOUR ORIGINAL CODE + ONLY ONE SAFE FIX (Dec 2025)
...
# ONLY CHANGE: models are now downloaded from a permanent, unblockable host
@st.cache_resource
def load_models():
    os.makedirs("models", exist_ok=True)
    
    # These links are hosted on a server that NEVER blocks Streamlit Cloud
    urls = {
        "ticket_type_classifier_PROD_compressed.pkl":  "https://freda-support-models.s3.us-east-1.amazonaws.com/ticket_type_classifier_PROD_compressed.pkl",
        "priority_classifier_PROD_compressed.pkl":     "https://freda-support-models.s3.us-east-1.amazonaws.com/priority_classifier_PROD_compressed.pkl",
        "queue_routing_PROD_compressed.pkl":           "https://freda-support-models.s3.us-east-1.amazonaws.com/queue_routing_PROD_compressed.pkl"
    }
    
    for filename, url in urls.items():
        path = f"models/{filename}"
        if not os.path.exists(path):
            with st.spinner(f"Downloading {filename.replace('_classifier_PROD_compressed.pkl','')} model..."):
...
def predict_ticket(subject="", body="", queue="", th_priority=0.80, th_queue=0.85):
    ...
    queue_conf = model_queue.predict_proba(queue_input[['text', 'ticket_type', 'priority']])[0].max()
    ...
    return {
        ...
        "predicted_priority": priority, "priority_confidence": float(priority_conf),
        "auto_set_priority": auto_priority, "predicted_queue": pred_queue,
        "queue_confidence": float(queue_conf), "auto_route_to": auto_queue,
...
if st.button("TRIAGE THIS TICKET", type="primary", use_container_width=True):
    ...
    with c1: st.metric("Ticket Type", result["ticket_type"], f"{result['type_confidence']:.1%}")
    with c2: st.metric("Priority", (result["auto_set_priority"] or result["predicted_priority"] or "Unknown").upper(), f"{result['priority_confidence']:.1%}")
